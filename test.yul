object "Token" {
    ...
    object "runtime" {
        code {   
            ...
            //require(iszero(callvalue())) commented out as 0.001 ether to mint

            // Dispatcher
            // All high-level functions
            switch selector()
            case 0x17f87bf7 /* "balanceOf(address, uint256)" */ {
                enforceNonPayable()
            ...
            case 0x7c4fbc34 /* "setApprovalForAll(address, bool)" */ {
                enforceNonPayable()
                // _setApprovalForAll(_msgSender(), operator, approved)
                _setApprovalForAll(caller(), decodeAsAddress(0), decodeAsBool(1))
            }
            case 0xab210f1b /* "mint(address,uint256,uint256)" */ {
                // check if they passed in 0.001 ether, else revert
                equalsMinMint()
            ...
            */
            ...
            // check if minMint value passed as callvalue function
            function equalsMinMint() {
                require(eq(minMint(), callvalue()))
            }
